...
class SystemApiDriver(SystemDriver):
    ...
    def login(self, request: LoginRequest) -> Result[str]:
    ...
    def get_current_user(self) -> Result[str]:
        return self._client.auth.get_current_user()
    ...
    def create_reservation(self, request: CreateReservationRequest) -> Result[ReservationResponse]:
        return self._client.reservation.create_reservation(
            request["car_id"], request["charging_point_id"], request["start_time"], request["end_time"]
        )
