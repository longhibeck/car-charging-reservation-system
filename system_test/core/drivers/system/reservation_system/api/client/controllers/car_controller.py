...
class CarController:
    ...
    def update_car(
        ...
        response: TypedResponse = self.http_client.put(
            f"{self.CARS_PATH}/{car_id}", request
        )
        return HttpTestUtils.get_ok_result_or_failure(response)

    def delete_car(self, car_id: str) -> Result[None]:
        response: TypedResponse = self.http_client.delete(f"{self.CARS_PATH}/{car_id}")
