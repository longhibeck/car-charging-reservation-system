...
class ChargingPointsController:
    ...
    def list_charging_points(self) -> Result[None]:
        response: TypedResponse = self._http_client.get(f"{self.ENDPOINT}/")
        result = HttpTestUtils.get_ok_result_or_failure(response)
        if result.is_failure():
            return Result.failure(result.get_error_messages())
        return Result.success()

    def get_charging_point(self, charging_point_id: str) -> Result[None]:
        response: TypedResponse = self._http_client.get(
            f"{self.ENDPOINT}/{charging_point_id}"
        )
        result = HttpTestUtils.get_ok_result_or_failure(response)
        if result.is_failure():
            return Result.failure(result.get_error_messages())
