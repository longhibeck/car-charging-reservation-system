...
class HomePage:
    ...
    def is_loaded(self) -> bool:
        ...
        return (
            self.page_client.is_visible_by_role("heading", name=self.YOUR_RESERVATIONS_HEADING) and
            self.page_client.is_visible_by_role("heading", name=self.YOUR_CARS_HEADING)
        )
    
    def has_your_reservations_heading(self) -> bool:
        """Check if 'Your Reservations' heading is visible"""
        return self.page_client.is_visible_by_role("heading", name=self.YOUR_RESERVATIONS_HEADING)
    
    def has_your_cars_heading(self) -> bool:
        """Check if 'Your Cars' heading is visible"""
        return self.page_client.is_visible_by_role("heading", name=self.YOUR_CARS_HEADING)
    
    def has_new_reservation_button(self) -> bool:
        """Check if 'New Reservation' button is visible"""
        return self.page_client.is_visible_by_role("button", name=self.NEW_RESERVATION_BUTTON)
    
    def has_reservation_history_button(self) -> bool:
        """Check if 'Reservation History' button is visible"""
        return self.page_client.is_visible_by_role("button", name=self.RESERVATION_HISTORY_BUTTON)

    # Assertion methods - for convenience (commonly used assertions)
    def assert_loaded(self):
        """Assert home page is loaded with all expected elements"""
        expect(self.page_client.page.get_by_role("heading", name=self.YOUR_RESERVATIONS_HEADING)).to_be_visible()
        expect(self.page_client.page.get_by_role("heading", name=self.YOUR_CARS_HEADING)).to_be_visible()
